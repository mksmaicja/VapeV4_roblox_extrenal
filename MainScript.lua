...
local function vapeGithubRequest(scripturl)
	if not isfile("VAPEzFORK/"..scripturl) then
		...
		suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/mikusgszyp/VapeV4_roblox_extrenal/"..readfile("VAPEzFORK/commithash.txt").."/"..scripturl, true) end)
		if not suc or res == "404: Not Found" then
			displayErrorPopup("Failed to connect to github : VAPEzFORK/"..scripturl.." : "..res)
			error(res)
		end
...
assert(not shared.VapeExecuted, "VAPEzFORK Already Injected")
shared.VapeExecuted = true

for i,v in pairs({baseDirectory:gsub("/", ""), "VAPEzFORK", "VAPEzFORK/Libraries", "VAPEzFORK/CustomModules", "VAPEzFORK/Profiles", baseDirectory.."Profiles", "VAPEzFORK/assets"}) do 
	if not isfolder(v) then makefolder(v) end
end
task.spawn(function()
	local success, assetver = pcall(function() return vapeGithubRequest("assetsversion.txt") end)
	if not isfile("VAPEzFORK/assetsversion.txt") then writefile("VAPEzFORK/assetsversion.txt", "0") end
	if success and assetver > readfile("VAPEzFORK/assetsversion.txt") then
		redownloadedAssets = true
		if isfolder("VAPEzFORK/assets") and not shared.VapeDeveloper then
			if delfolder then
				delfolder("VAPEzFORK/assets")
				makefolder("VAPEzFORK/assets")
			end
		end
		writefile("VAPEzFORK/assetsversion.txt", assetver)
	end
end)
if not isfile("VAPEzFORK/CustomModules/cachechecked.txt") then
	local isNotCached = false
	for i,v in pairs({"VAPEzFORK/Universal.lua", "VAPEzFORK/MainScript.lua", "VAPEzFORK/GuiLibrary.lua"}) do 
	...
	if isfolder("VAPEzFORK/CustomModules") then 
		for i,v in pairs(listfiles("VAPEzFORK/CustomModules")) do 
			if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
				isNotCached = true
			end 
		end
	...
	if isNotCached and not shared.VapeDeveloper then
		displayErrorPopup("VAPEzFORK has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			for i,v in pairs({"VAPEzFORK/Universal.lua", "VAPEzFORK/MainScript.lua", "VAPEzFORK/GuiLibrary.lua"}) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					delfile(v)
				end 
			...
			for i,v in pairs(listfiles("VAPEzFORK/CustomModules")) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/mikusgszyp/VapeV4_roblox_extrenal/"..readfile("VAPEzFORK/commithash.txt").."/CustomModules/"..last) end)
					if suc and publicrepo and publicrepo ~= "404: Not Found" then
						writefile("VAPEzFORK/CustomModules/"..last, "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					end
				end 
			end
		end})
	end
	writefile("VAPEzFORK/CustomModules/cachechecked.txt", "verified")
end
...
task.spawn(function()
	...
	image.Image = downloadVapeAsset("VAPEzFORK/assets/CombatIcon.png")
	...
	task.spawn(function()
		task.wait(15)
		if image and image.ContentImageSize == Vector2.zero and (not errorPopupShown) and (not redownloadedAssets) and (not isfile("VAPEzFORK/assets/check3.txt")) then 
            errorPopupShown = true
            displayErrorPopup("Assets failed to load, Try another executor (executor : "..(identifyexecutor and identifyexecutor() or "Unknown")..")", {OK = function()
                writefile("VAPEzFORK/assets/check3.txt", "")
            end})
        end
	end)
end)
...
local Combat = GuiLibrary.CreateWindow({
	Name = "Combat", 
	Icon = "VAPEzFORK/assets/CombatIcon.png", 
	IconSize = 15
})
local Blatant = GuiLibrary.CreateWindow({
	Name = "Blatant", 
	Icon = "VAPEzFORK/assets/BlatantIcon.png", 
	IconSize = 16
})
local Render = GuiLibrary.CreateWindow({
	Name = "Render", 
	Icon = "VAPEzFORK/assets/RenderIcon.png", 
	IconSize = 17
})
local Utility = GuiLibrary.CreateWindow({
	Name = "Utility", 
	Icon = "VAPEzFORK/assets/UtilityIcon.png", 
	IconSize = 17
})
local World = GuiLibrary.CreateWindow({
	Name = "World", 
	Icon = "VAPEzFORK/assets/WorldIcon.png", 
	IconSize = 16
})
local Friends = GuiLibrary.CreateWindow2({
	Name = "Friends", 
	Icon = "VAPEzFORK/assets/FriendsIcon.png", 
	IconSize = 17
})
...
local Profiles = GuiLibrary.CreateWindow2({
	Name = "Profiles", 
	Icon = "VAPEzFORK/assets/ProfilesIcon.png", 
	IconSize = 19
})
GUI.CreateDivider()
GUI.CreateButton({
...
OnlineProfilesButtonImage.Image = downloadVapeAsset("VAPEzFORK/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = downloadVapeAsset("VAPEzFORK/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = downloadVapeAsset("VAPEzFORK/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = downloadVapeAsset("VAPEzFORK/assets/ProfilesIcon.png")
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/mikusgszyp/VapeV4_roblox_extrenal/main/Profiles/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/mikusgszyp/VapeV4_roblox_extrenal/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
...
local TextGUI = GuiLibrary.CreateCustomWindow({
	Name = "Text GUI", 
	Icon = "VAPEzFORK/assets/TextGUIIcon1.png", 
	IconSize = 21
})
local TextGUICircleObject = {CircleList = {}}
GUI.CreateCustomToggle({
	Name = "Text GUI", 
	Icon = "VAPEzFORK/assets/TextGUIIcon3.png",
...
VapeLogo.Image = downloadVapeAsset("VAPEzFORK/assets/VapeLogo3.png")
...
VapeLogoV4.Image = downloadVapeAsset("VAPEzFORK/assets/VapeLogo4.png")
...
TextGUIMode = TextGUI.CreateDropdown({
	...
	Function = function(val)
		...
		if val == "Drawing" then
			...
			VapeLogoDrawing.Data = readfile("VAPEzFORK/assets/VapeLogo3.png")
			...
			VapeLogoV4Drawing.Data = readfile("VAPEzFORK/assets/VapeLogo4.png")
			...
			VapeLogoShadowDrawing.Data = readfile("VAPEzFORK/assets/VapeLogo3.png")
			...
			VapeLogo4Drawing.Data = readfile("VAPEzFORK/assets/VapeLogo4.png")
...
local TargetInfo = GuiLibrary.CreateCustomWindow({
	Name = "Target Info",
	Icon = "VAPEzFORK/assets/TargetInfoIcon1.png",
	IconSize = 16
})
...
TargetInfoHealthBackgroundShadow.Image = downloadVapeAsset("VAPEzFORK/assets/WindowBlur.png")
...
GUI.CreateCustomToggle({
	Name = "Target Info", 
	Icon = "VAPEzFORK/assets/TargetInfoIcon2.png", 
    ...
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			...
			if shared.VapeDeveloper then 
				loadstring(readfile("VAPEzFORK/NewMainScript.lua"))() 
			else 
				loadstring(game:HttpGet("https://raw.githubusercontent.com/mikusgszyp/VapeV4_roblox_extrenal/"..readfile("VAPEzFORK/commithash.txt").."/NewMainScript.lua", true))() 
			end
		]]
		if shared.VapeDeveloper then
...
local function loadVape()
	if not shared.VapeIndependent then
		...
		if isfile("VAPEzFORK/CustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("VAPEzFORK/CustomModules/"..game.PlaceId..".lua"))()
		else
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/mikusgszyp/VapeV4_roblox_extrenal/"..readfile("VAPEzFORK/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
					writefile("VAPEzFORK/CustomModules/"..game.PlaceId..".lua", "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
					loadstring(readfile("VAPEzFORK/CustomModules/"..game.PlaceId..".lua"))()
				end
			end
		end
		--if shared.VapePrivate then
		if isfile("VAPEzFORKprivate/CustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("VAPEzFORKprivate/CustomModules/"..game.PlaceId..".lua"))()
		end	
		--end
	else
	...
	if not shared.VapeSwitchServers then
		if BlatantModeToggle.Enabled then
			pcall(function()
				local frame = GuiLibrary.CreateNotification("Blatant Enabled", "VAPEzFORK is now in Blatant Mode.", 5.5, "assets/WarningNotification.png")
